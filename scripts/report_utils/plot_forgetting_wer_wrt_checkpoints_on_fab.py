...
def main(filepath: str, is_relative: bool=False):
    ...
    # Save figure:
    if is_relative:
        savepath = (DEFAULT_OUTPUT_DIR / "compare_wer_multiple_models" / (Path(filepath).stem + "-relative")).with_suffix(".png")
    else:
        savepath = (DEFAULT_OUTPUT_DIR / "compare_wer_multiple_models" / Path(filepath).stem).with_suffix(".png")
