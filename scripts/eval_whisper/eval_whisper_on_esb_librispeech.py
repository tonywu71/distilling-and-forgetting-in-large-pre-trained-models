...
def main(pretrained_model_name_or_path: str,
    ...
    wandb.init(project=os.environ["WANDB_PROJECT"],
               job_type="evaluation",
               name=f"eval_esb_librispeech-{extract_exp_name_from_model_path(pretrained_model_name_or_path)}",
               config=config)
    ...
    # Save results:
    if savepath is None:
        savepath = extract_output_savepath_from_model_path(pretrained_model_name_or_path) + "-esb_librispeech.csv"
