...
from dataloader.datasets.librispeech_clean_validation import LibriSpeechCleanValidation
...
def main(pretrained_model_name_or_path: str=typer.Argument(..., help="Path to the pretrained model or its name in the HuggingFace Hub."),
         ...
         subset: Optional[List[str]]=typer.Option(None, help="Subset of the LibriSpeech clean validation dataset to evaluate on."),
    ...
    """
    Evaluate the whisper model on the LibriSpeech clean validation set.
    ...
    wandb.init(project=os.environ["WANDB_PROJECT"],
               job_type="evaluation",
               name=f"eval_librispeech_clean_val-{extract_experiment_name(pretrained_model_name_or_path)}",
               config=config)
    ...
    librispeech_dataset = LibriSpeechCleanValidation(streaming=streaming, subset=subset)
    print(f"Loaded datasets: {list(librispeech_dataset.keys())}")
    
    
    # Preprocess:
    print("Preprocessing datasets...")
    librispeech_dataset.preprocess_datasets(normalize=True)
    
    
    # Evaluate:
    print("Evaluating...")
    results = eval_whisper_on_dataset(pretrained_model_name_or_path=pretrained_model_name_or_path,
                                      ds_group=librispeech_dataset,
                                      batch_size=batch_size,
                                      task=task)
